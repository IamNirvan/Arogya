    ...
    <?php
        if($_SESSION["accountType"] == "receptionist") {
            ...
            if(isset($_GET["searchButton"])) {
                $option = $_GET['searchOption'];
                $term = $_GET['searchBar'];
                $tableDataQuery = "SELECT * FROM operatingroomschedule WHERE $option = '$term' AND 
                                          employeeID = '$doctorID';";
            }
            else {
            ...
            $attributes1 = ["operatingRoomScheduleID", "bookedDate", "startTime", "endTime", "operatingRoomID",
                            "patientID"];
            addTable(columnNames: $columnNames1, attributes: $attributes1, query:$tableDataQuery,
                title:"Operating Room Bookings", size:"largeBox", addItemPage: "addORBooking.php?ID=$doctorID",
                deleteLink: "deleteORBooking.php", updateLink: "updateORBooking.php", viewLink: $_SERVER['PHP_SELF']);
            echo '</div>';
        }
    ?>
</main>
